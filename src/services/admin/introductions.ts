...
export interface IntroductionUploadOptions {
  maxSize?: number; // MB 단위
  allowedTypes?: string[];
}

export class IntroductionService extends StorageService {
  private static readonly BUCKET_NAME = "documents";
  private static readonly FOLDER_NAME = "introduction";

  // 회사 소개서 파일 업로드
  async uploadIntroduction(
    file: File,
    options: IntroductionUploadOptions = {}
  ): Promise<UploadResult> {
    const defaultOptions: FileUploadOptions = {
      bucket: IntroductionService.BUCKET_NAME,
      folder: IntroductionService.FOLDER_NAME,
  ...
  async getIntroductions(): Promise<Introduction[]> {
    try {
      const files = await this.listFiles(
        IntroductionService.BUCKET_NAME,
        IntroductionService.FOLDER_NAME,
        {
          sortBy: "created_at",
        }
      );
      ...
      return files.map((file) => ({
        ...
        url: this.getPublicUrl(
          IntroductionService.BUCKET_NAME,
          `${IntroductionService.FOLDER_NAME}/${file.name}`
        ),
        path: `${IntroductionService.FOLDER_NAME}/${file.name}`,
        uploadedAt:
  ...
  // 최신 회사 소개서 파일 조회
  async getLatestIntroduction(): Promise<Introduction | null> {
    try {
      const documents = await this.getIntroductions();
  ...
  // 파일 삭제
  async deleteIntroduction(fileName: string): Promise<void> {
    try {
      const filePath = `${IntroductionService.FOLDER_NAME}/${fileName}`;
      await this.deleteFile(IntroductionService.BUCKET_NAME, filePath);
    } catch (error) {
