...
import { Plus, Search, Edit2, Trash2, Eye, Filter, Tag, Calendar, User, BarChart3, FolderOpen, Upload, X } from 'lucide-react';
...
export default function EventsPage() {
  ...
  const fetchPosts = async () => {
    try {
      ...
      // 통계 조회
      const statsData = await postsService.getPostStats('events');
      setStats(statsData);
    } catch (error) {
  ...
  const fetchCategories = async () => {
    try {
      const data = await postsService.getCategories('events');
      setCategories(data);
    } catch (error) {
